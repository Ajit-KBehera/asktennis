...
class CSVDataLoader {
  ...
  /**
   * Load all CSV data into database using streaming approach
   */
  async loadAllData() {
    if (this.isLoading) {
    ...
    try {
      ...
      // Load the complete dataset using streaming
  ...
  /**
   * Load the complete tennis dataset using streaming approach
   */
  async loadCompleteDatasetStreaming() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const batchSize = 50; // Very small batches for memory efficiency
      let batch = [];
  ...
  /**
   * Process a batch of records with streaming approach
   */
    ...
    for (const record of batch) {
      try {
        ...
        if (this.isMatchRecord(record)) {
          await this.processMatchRecordStreaming(client, record);
        } else if (this.isPlayerRecord(record)) {
          await this.processPlayerRecordStreaming(client, record);
        } else if (this.isRankingRecord(record)) {
          await this.processRankingRecordStreaming(client, record);
        }
      } catch (error) {
  ...
  /**
   * Process a single match record with streaming approach
   */
  async processMatchRecordStreaming(client, record) {
    try {
      await client.query(`
  ...
  /**
   * Process a single player record with streaming approach
   */
  async processPlayerRecordStreaming(client, record) {
    try {
      await client.query(`
        INSERT INTO players (
  ...
  /**
   * Process a single ranking record with streaming approach
   */
  async processRankingRecordStreaming(client, record) {
    try {
